...
def _reconcile_new_out_of_domain_rows(
    ...
    try:
        ...
        try:
            ...
            keys = [int(row[0]) for row in cur.fetchall()]
            ...
            return {
                ...
                "rows_found": len(keys),
                "request_keys": keys,
                "fact_updated": len(keys),
                "bridge_delete_statements": len(keys),
            }
...
def _ask_route_is_wrapped(app) -> bool:
    for route in app.router.routes:
        if not isinstance(route, APIRoute):
            continue
        if route.path != "/ask" or "POST" not in set(route.methods or set()):
            continue
        return bool(
            getattr(
                route.endpoint,
...
def _install_runtime_marker(app) -> None:
    for route in app.router.routes:
        if getattr(route, "path", None) == "/demo/grounding-runtime":
            return

    @app.get("/demo/grounding-runtime")
    def grounding_runtime_marker():
        return {
            "version": GUARD_RUNTIME_VERSION,
            "pid": os.getpid(),
            "ask_guard_wrapped": _ask_route_is_wrapped(app),
        }


def install_grounding_guard(app) -> None:
    already_wrapped = _ask_route_is_wrapped(app)
