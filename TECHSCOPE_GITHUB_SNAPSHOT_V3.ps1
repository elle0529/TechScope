...
Write-Host ""
Write-Host "TechScope GitHub Snapshot v3"
Write-Host "AST sanitizer -> secret scan -> private GitHub push"
Write-Host ""

if(-not (Test-Path $RepoRoot)){ throw "TECHSCOPE_ROOT_NOT_FOUND" }

Copy-Item `
    (Join-Path $PSScriptRoot "_techscope_payload\tools\sanitize_p1d_sql_verify_v3.py") `
    (Join-Path $RepoRoot "tools\sanitize_p1d_sql_verify_v3.py") `
    -Force

python.exe (Join-Path $RepoRoot "tools\sanitize_p1d_sql_verify_v3.py")
        ...
        Get-Content -LiteralPath $file.FullName -ErrorAction Stop | ForEach-Object {
            ...
            foreach($rule in $rules){
                if($line-match $rule.Pattern){
                    # Explicitly allow environment variable *names*.
                    if(
