...
Write-Host ""
Write-Host "TechScope P1E Relation Repair Resume v6"
Write-Host "Alias resolution + local preflight + Serverless Spark"
Write-Host ""
...
Copy-Item `
    (Join-Path $PSScriptRoot "_techscope_payload\tools\patch_p1e_relation_repair_v6.py") `
    (Join-Path $Repo "tools\patch_p1e_relation_repair_v6.py") `
    -Force
...
& docker.exe --context $Ctx exec `
    ...
    $Container `
    python /workspaces/TechScope/tools/patch_p1e_relation_repair_v6.py

if($LASTEXITCODE-ne 0){ throw "P1E_V6_PATCH=FAIL" }
...
if($LASTEXITCODE-ne 0){ throw "P1E_V6_COMPILE=FAIL" }

Write-Host "P1E_V6_COMPILE=PASS"
Write-Host "P1E_RELATION_REPAIR=RESUME_ALIAS_AWARE"
...
if($LASTEXITCODE-ne 0){
    throw "P1E_RELATION_REPAIR_V6=FAIL"
}
