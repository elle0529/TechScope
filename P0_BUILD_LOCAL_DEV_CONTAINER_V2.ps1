...
Write-Host ""
Write-Host "TechScope P0 Local Dev Container Build v2 FIXED"
...
# ----------------------------------------------------------------------
# 1. Install repository files from isolated payload directory
# ----------------------------------------------------------------------
...
foreach ($sourceName in $copyMap.Keys) {
    $source = Join-Path $PayloadRoot $sourceName
    ...
    if ($sourceResolved -ieq $targetResolved) {
        Write-Host ("KEEP    " + $copyMap[$sourceName])
        continue
    }
...
# ----------------------------------------------------------------------
# 4. Reuse or replace only TechScope-owned stale container
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 7. Repository entry-point smoke
# ----------------------------------------------------------------------
