...
app.on('message', async ({ send, activity }) => {
    ...
    const sessionId = textValue(activity.conversation?.id).trim() ||
        `teams-${Date.now()}`;
    const userId = textValue(activity.from?.id).trim() ||
        'teams-user';
    try {
        ...
        const technologyIds = Array.isArray(result.grounded_technology_ids)
            ...
            : [];
        const persisted = response.headers.get('x-techscope-cosmos-persisted') || 'unknown';
        const interactionId = response.headers.get('x-techscope-interaction-id') || '';
        ...
        await send('TechScope 요청 처리 중 오류가 발생했습니다. 로컬 FastAPI 및 Dev Tunnel 상태를 확인해 주세요.');
    }
});
app
