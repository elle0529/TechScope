...
Write-Host ""
Write-Host "TechScope P1D Databricks + Azure SQL Resume v4"
...
$files=@(
 ...
 "tools\validate_p1d_v4.py"
)
foreach($f in $files){Install-File $f}

Write-Host ""
Write-Host "P1D_V4_COMPILE=START"
$code=Invoke-Native {
 docker exec --user vscode $ContainerName python -m py_compile `
  ...
  /workspaces/TechScope/tools/validate_p1d_v4.py
}
if($code -ne 0){exit $code}
Write-Host "P1D_V4_COMPILE=PASS"

Write-Host ""
Write-Host "P1D_V4_STATIC_VALIDATION=START"
$code=Invoke-Native {
 docker exec --user vscode $ContainerName `
  python /workspaces/TechScope/tools/validate_p1d_v4.py
}
if($code -ne 0){exit $code}

Write-Host ""
Write-Host "P1D_V4_RESUME=START"
$code=Invoke-Native {
...
if($code -ne 0){
 Write-Host ("P1D_V4_RESUME=FAIL EXIT="+$code)
 exit $code
}

Write-Host ""
Write-Host "P1D_V4_DOC_SYNC=START"
$code=Invoke-Native {
 docker exec --user vscode $ContainerName `
  python /workspaces/TechScope/tools/sync_p1d_docs.py
}
if($code -ne 0){exit $code}
Write-Host "P1D_V4_DOC_SYNC=PASS"
...
Write-Host ""
Write-Host "P1D_V4_PACKAGE_EXECUTION=PASS"
