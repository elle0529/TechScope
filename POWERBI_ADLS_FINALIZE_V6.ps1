...
Write-Host ""
Write-Host "TechScope Power BI ADLS Finalize v6"
Write-Host "Uses actual extractor schema + canonical RAG Domain Evidence."
Write-Host "No fake values. No upstream pipeline rerun."
Write-Host ""
...
Copy-Item (Join-Path $Payload "tools\powerbi_adls_finalize_v6.py") `
    (Join-Path $Repo "tools\powerbi_adls_finalize_v6.py") -Force
Write-Host "POWER_BI_ADLS_FINALIZE_V6_TOOL=INSTALLED"

Write-Host "POWER_BI_ADLS_FINALIZE_V6=START"
Run-Docker @(
    "exec","--user","vscode",
    "-w","/workspaces/TechScope",
    "-e","PYTHONPATH=/workspaces/TechScope",
    $Container,
    "python","/workspaces/TechScope/tools/powerbi_adls_finalize_v6.py"
)

Write-Host "ARCHITECTURE_LINT_AFTER_POWER_BI_V6=START"
Run-Docker @(
    "exec","--user","vscode",
    "-w","/workspaces/TechScope",
    "-e","PYTHONPATH=/workspaces/TechScope",
...
Write-Host "ARCHITECTURE_LINT_AFTER_POWER_BI_V6=PASS"
