...
function Write-RebootAction {
    $lines = @(
        "# Manual Actions",
        "",
        "blocked_stage: P0 Bootstrap / WSL readiness",
        "affected_component: Automation & Operations Plane",
        "reason: WSL installation/update completed but Windows restart is required before WSL can be validated.",
        "where_to_fix: Windows",
        "exact_manual_action: Restart Windows once. After login, run .\RUN_P0_MINIMAL_HOST_BOOTSTRAP_V3.cmd again from C:\TechScope.",
        "how_to_verify: The same script reaches WSL_READY=PASS and continues automatically.",
        "resume_path_or_command: .\RUN_P0_MINIMAL_HOST_BOOTSTRAP_V3.cmd"
    )
...
function Write-VirtualizationAction {
    $lines = @(
        "# Manual Actions",
        "",
        "blocked_stage: P0 Bootstrap / WSL readiness",
        "affected_component: Automation & Operations Plane",
        "reason: CPU virtualization is disabled in firmware.",
        "where_to_fix: BIOS/UEFI firmware settings",
        "exact_manual_action: Enable Intel Virtualization Technology (VT-x/VT-d) or AMD SVM/AMD-V in BIOS/UEFI, save, and boot Windows normally.",
        "how_to_verify: Run the same bootstrap script; WSL setup must continue without FIRMWARE_VIRTUALIZATION=OFF.",
        "resume_path_or_command: .\RUN_P0_MINIMAL_HOST_BOOTSTRAP_V3.cmd"
    )
...
function Write-WslFailureAction {
    $logPath = "results\latest\wsl-bootstrap.log"
    ...
    $lines = @(
        ...
        "blocked_stage: P0 Bootstrap / WSL readiness",
        "affected_component: Automation & Operations Plane",
        "reason: Microsoft wsl.exe install/update failed.",
        "where_to_fix: WSL prerequisite",
        "exact_manual_action: Do not install additional tools manually. Send " + $logPath + " to ChatGPT.",
        "how_to_verify: The repaired WSL unit reaches WSL_READY=PASS.",
        "resume_path_or_command: Determined after reading " + $logPath
    )
...
Write-Host ""
Write-Host "TechScope P0 Minimal Host Bootstrap v3"
...
# ----------------------------------------------------------------------
# 1. Reuse already-ready Docker first
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 2. WSL 2.1.5+ prerequisite
# ----------------------------------------------------------------------

$minimumWsl = [version]"2.1.5.0"
$currentWsl = Get-WslVersionObject

if (($null -eq $currentWsl) -or ($currentWsl -lt $minimumWsl)) {
    Write-Host "WSL_READY=PENDING"
    Write-Host "One Windows administrator approval may appear."

    if (-not (Test-Path $AdminScript)) {
        throw "P0_WSL_ADMIN_SETUP.ps1 파일이 없습니다."
    }
    ...
    if ($adminProcess.ExitCode -eq 40) {
        Write-VirtualizationAction
        Write-Host ""
        Write-Host "WSL_SETUP=BLOCKED"
        Write-Host "FIRMWARE_VIRTUALIZATION=OFF"
        exit 40
    }

    if ($adminProcess.ExitCode -ne 0) {
        Write-WslFailureAction
        Write-Host ""
        Write-Host ("WSL_SETUP=FAIL EXIT=" + $adminProcess.ExitCode)
        Write-Host "LOG=results\latest\wsl-bootstrap.log"
...
if (-not [string]::IsNullOrWhiteSpace($dockerDesktop)) {
    ...
    # The Docker service/subscription agreement is NOT auto-accepted here.
...
if (-not (Test-DockerReady)) {
    ...
    if (-not $ready) {
        $lines = @(
            ...
            "affected_component: Automation & Operations Plane",
            "reason: Docker Desktop is installed but the Docker daemon did not become ready within 5 minutes.",
            "where_to_fix: Docker Desktop",
            "exact_manual_action: Open Docker Desktop once. If its service/subscription agreement is shown, review it and decide whether to accept it. Then wait until Docker reports Running.",
            "how_to_verify: docker info succeeds.",
            "resume_path_or_command: .\RUN_P0_MINIMAL_HOST_BOOTSTRAP_V3.cmd"
        )
